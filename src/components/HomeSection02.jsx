import React from "react";

import Img01 from "../assets/Img01.png";

const HomeSection02 = () => {
  return (
    <div className="my-20">
      <div className="flex flex-col lg:flex-row justify-around items-center my-14">
        <div className="flex flex-col gap-5 w-[90%] lg:w-[45%]">
          <h1 className="text-3xl text-gray-700 font-bold dark:text-gray-100">
            Join our community in creating a more sustainable future for
            everyone
          </h1>
          <p className="text-lg md:text-xl lg:text-lg dark:text-gray-300">
            At Ecoyaan, we are more than just a platform. Our vision is to build
            an eco-conscious community for people and products.
          </p>
          <p className="text-lg md:text-xl lg:text-lg dark:text-gray-300">
            Consider us your partner for all things sustainable. To keep you
            motivated on the journey, we will provide you with engaging
            information about climate change, the latest updates on climate
            policies and lifestyle tips that you can adopt to reduce your impact
            on the environment.
          </p>
        </div>

        <div className="w-[90%] lg:w-[40%]">
          <img src={Img01} alt="" />
        </div>
      </div>

      <div className="flex flex-col lg:flex-row justify-around items-center my-14">
        <div className="flex flex-col gap-5 w-[90%] lg:w-[45%]">
          <h1 className="text-3xl text-gray-700 font-bold dark:text-gray-100">
            Join our community in creating a more sustainable future for
            everyone
          </h1>
          <p className="text-lg md:text-xl lg:text-lg dark:text-gray-300">
            At Ecoyaan, we are more than just a platform. Our vision is to build
            an eco-conscious community for people and products.
          </p>
          <p className="text-lg md:text-xl lg:text-lg dark:text-gray-300">
            Consider us your partner for all things sustainable. To keep you
            motivated on the journey, we will provide you with engaging
            information about climate change, the latest updates on climate
            policies and lifestyle tips that you can adopt to reduce your impact
            on the environment.
          </p>
        </div>

        <div className="w-[90%] lg:w-[40%]">
          <img src={Img01} alt="" />
        </div>
      </div>

      <div className="flex flex-col lg:flex-row justify-around items-center my-14">
        <div className="flex flex-col gap-5 w-[90%] lg:w-[45%]">
          <h1 className="text-3xl text-gray-700 font-bold dark:text-gray-100">
            Join our community in creating a more sustainable future for
            everyone
          </h1>
          <p className="text-lg md:text-xl lg:text-lg dark:text-gray-300">
            At Ecoyaan, we are more than just a platform. Our vision is to build
            an eco-conscious community for people and products.
          </p>
          <p className="text-lg md:text-xl lg:text-lg dark:text-gray-300">
            Consider us your partner for all things sustainable. To keep you
            motivated on the journey, we will provide you with engaging
            information about climate change, the latest updates on climate
            policies and lifestyle tips that you can adopt to reduce your impact
            on the environment.
          </p>
        </div>

        <div className="w-[90%] lg:w-[40%]">
          <img src={Img01} alt="" />
        </div>
      </div>
    </div>
  );
};

export default HomeSection02;
